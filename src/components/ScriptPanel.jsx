...
const CHECKBOX_GRID_SCRIPT_IDS = [
  'domain-migration-smart-backup',
  'disk-cleanup',
  'clear-ie-cache',
        ...
        <div className="max-w-6xl mx-auto p-8 space-y-8">
          ...
          <div className="bg-dark-surface rounded-xl p-6 border border-dark-border">
              ...
              {script.id === 'standard-apps-installer' && script.inputs.some(i => i.type === 'checkbox') && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={() => {
                      const updates = {};
                      script.inputs.filter(i => i.type === 'checkbox').forEach(i => { updates[i.variable] = 'true'; });
                      setInputs(prev => ({ ...prev, ...updates }));
                    }}
                    className="flex items-center gap-2 px-3 py-1.5 bg-emerald-900/50 hover:bg-emerald-800/50 text-emerald-300 rounded-lg text-sm font-medium transition-colors border border-emerald-700"
                  >
            ...
            <div className={`grid gap-4 ${
              CHECKBOX_GRID_SCRIPT_IDS.includes(script.id)
                ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'
                : 'grid-cols-1 md:grid-cols-2'
            }`}>
              {script.inputs.map(input => (
                <div key={input.variable} className={(input.type === 'checkbox' && CHECKBOX_GRID_SCRIPT_IDS.includes(script.id)) ? '' : 'col-span-full'}>
                  <InputField
                    input={input}
