...
function createWindow() {
  ...
  if (process.env.NODE_ENV === 'development' || !app.isPackaged) {
    mainWindow.loadURL('http://localhost:5457');
...
function getDefaultScriptsPath() {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, 'scripts.json');
  }
  return path.join(__dirname, '..', 'src', 'data', 'scripts.json');
}

function isSectionedCategory(catData) {
...
function mergeScriptsData(userData, defaultData) {
...
ipcMain.handle('scripts:getData', async () => {
  try {
    ...
    return mergeScriptsData(user, defaults);
  } catch (err) {
